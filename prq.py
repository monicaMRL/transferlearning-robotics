...
wval_source = list()
new_count = 0
old_count = {0:0, 1:0, 2:0, 3:0}
chosen_old = dict()
chosen_ind = 0

def read_oldQ(filenames):
    global old_policyList, w_source
    i = 0
    for f in filenames:
        q = open(f,'r')
        ...
        old_policyList.append(stateAction_old)
        w_source[i] =  0
        wval_source.append(0)
        i += 1
...
def selection_probability():
    ...
    for i in range(len(w_source.keys())):
        summ += math.e**(temp_para*w_source[i])
        
    for j in range(len(w_source.keys())):
        p = math.e**(temp_para*w_source[j])/summ
        
        probability_p.append(p)
...
def timerHandler():
    ...
    global chosen_old, chosen_ind, old_count, temp_para, w_source
    ...
    if not(stepCount >= maxSteps or (current_state == end)):
        ...
        if not (epi_count >= max_epi):
            ...
            w_source[chosen_ind] = (w_source[chosen_ind] * old_count[chosen_ind] + averageReward)/(old_count[chosen_ind] + 1)
            old_count[chosen_ind] += 1
            temp_para += temp_inc
            ...
            select_p = selection_probability()
            chosen_ind = np.where(np.random.multinomial(1,select_p))[0][0]
            chosen_old = old_policyList[chosen_ind]
            
            aveRlist.append(averageReward)
            
            aveR = open('aveR_prq.txt','a+')
...
fileNames = ['pi1.txt','pi2.txt','pi3.txt','pi4.txt']

initialise_Qsa()
read_oldQ(fileNames)
chosen_old = old_policyList[0]
